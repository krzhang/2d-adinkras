...
\end{tikzpicture}
\end{center}
\caption{An adjacent swap of colors.  If we swap colors in position $j$ and $j+1$, vertex $v_j$ will be replaced by vertex $v_j{}'$, and the path gets modified from the upper path to the lower path in the diagram.  The parity of the path changes by exactly $1$ because the square above has odd parity.
\label{fig:colorswap}}
\end{figure}
