...
pulls back the dashing to $\mu_2$ on $A_L^0\times A_R^0$. Note that Theorem~\ref{thm:dashing-under-quotient} can be reformulated as saying that $\mu_1$ and $\mu_2$ are equivalent under vertex switching. Our strategy will be to reduce the problem to checking a set of local conditions on cycles. We now give some definitions and auxiliary results for our approach.
...
Theorem~\ref{thm:dashing-under-quotient} tells us that our quotient, which did not a priori play well with dashing, actually respects dashing if we allow the action of vertex switching.
