...
\begin{cor}
\label{cor:knonzero}
Other than the zero vector, every element $\vec{x}$ of $K$ has non-zero $\pi_L(\vec{x})$ and $\pi_R(\vec{x})$.
\end{cor}
\begin{proof}
Suppose $\vec{x}\in K$ and $\pi_L(\vec{x})=\vec{0}$.  Then $\Psi(\vec{x})=\overline{0}$.  By Theorem~\ref{thm:kv0}, this means $\vec{x}=\vec{0}$.
\end{proof}
...
0&0&1&1&1&1
\end{array}\right]\]
Note that Corollary~\ref{cor:knonzero} is affirmed here since this has $1$s on both sides of the vertical line.  Another choice for $K$ would have been the code generated by
\[\left[\begin{array}{cccc|cc}
1&1&0&0&1&1
\end{array}\right]\]
...
\end{lem}
\begin{proof}
Suppose $(v,w)$ and $(v',w)$ are in the same connected component of $A'_L$ and $\Phi(v,w)=\Phi(v',w)$.  Then there is an $\vec{x}\in K$ so that $\vec{x}(v,w)=(v',w)$.  Let $\vec{y}\in \ZZ_2^p$ so that $\vec{y}v=v'$.  Then $\vec{x}$ and $Z_L(\vec{y})$ both send $(v,w)$ to $(v',w)$.  Since the group action is faithful, $\vec{x}=Z_L(\vec{y})$. By Corollary~\ref{cor:knonzero}, we have $\vec{x}=\vec{0}$, and so $v'=v$.
\end{proof}
