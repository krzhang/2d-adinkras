...
In fact, there is a map $j$ from the orbit space $E/C$ to the set of edges just defined, sending the orbit of $(v,w)$ to $(Cv,Cw)$.  That this map is surjective is clear from the definition.  But this map is, in general, not injective.  The problem is that a group element may connect two neighbors of a vertex, and thereby create a multiple edge.  Multiple edges are not profoundly problematic, but the formalism we are using to describe graphs does not allow it and it is inconvenient to alter the formalism to accommodate these features.  Fortunately, for the group actions described in this paper, these situations cannot arise.
...
\begin{prop}
\label{prop:distinject}
If $C$ is a distant group action, then the map $j$ is injective.
\end{prop}
\begin{proof}
Suppose $(v,w)$ and $(v',w')$ are edges in $E$ and suppose $(Cv,Cw)=(Cv',Cw')$.  Then there are group elements $g_1$ and $g_2\in C$ so that $g_1v=v'$ and $g_2w=w'$.  On the other hand, there is an edge $(g_1v,g_1w)=(v',g_1w)$.  So $(v',g_1w)$ and $(v',g_2w)=(v',w')$ are edges in the graph.  Then $g_2g_1^{-1}$ is a group element that sends $g_1w$ to $g_2w$ but we have just constructed a path of length $2$ from $g_1w$ to $g_2w$.  Since the group action is distant, we know that $g_2g_1^{-1}=1$ so that $g_1=g_2$.  Therefore $(v',w')=g_1(v,w)$.
\end{proof}

The benefit to this comes when our edges have colors and the group preserves colors:
...
\end{prop}
\begin{proof}
Since Proposition~\ref{prop:distinject} says that the edges of $A/C$ can be identified with $E/C$, and since the action of $C$ preserves colors, we can define the coloring on $E/C$, and thus, on the edges of $A/C$.
