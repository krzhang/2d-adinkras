...
\com{Make this not a lemma?  Also, introduce $C'$ notation?  Prove that  $Z_L(C(A_L^0))$ is $C(A)\cap Z_L(\ZZ_2^p)$, etc.}
\begin{lem}
\label{lem:cplus}
...
\[I^n/C(A) \cong A\]
and
\[I^n/(Z_L(C(A_L^0))\oplus Z_R(C(A_R^0)))\cong A_L^0\times A_R^0.\]
These isomorphisms can be chosen so that $\vec{0}=(0,\ldots,0)$ is sent to $\overline{0}$ in $A$ and $(\overline{0},\overline{0})$ in $A_L^0\times A_R^0$, respectively.

Define the quotient homomorphism
\[\pi:I^n/(Z_L(C(A_L^0))\oplus Z_R(C(A_R^0))) \to
(I^n/(Z_L(C(A_L^0))\oplus Z_R(C(A_R^0))))/K.\]
A standard argument gives
\[(I^n/(Z_L(C(A_L^0))\oplus Z_R(C(A_R^0))))/K
\cong I^n/(Z_L(C(A_L^0))\oplus Z_R(C(A_R^0))\oplus K)\]
and since
\[C(A)=Z_L(C(A_L^0))\oplus Z_R(C(A_R^0))\oplus K\]
we can take $\pi$ as
\[\pi:I^n/(Z_L(C(A_L^0))\oplus Z_R(C(A_R^0)))\to I^n/C(A).\]
We then have the following commutative diagram
\[
\begin{CD}
I^n/(Z_L(C(A_L^0))\oplus Z_R(C(A_R^0))) @>i_1>\cong> A_L^0\times A_R^0\\
@VV\pi V @VV\Phi V\\
I^n/C(A) @>i_2>\cong> A
\end{CD}
\]
...
Since $Z_L(\vec{x}_1)\overline{0}$ in $A$ is the same as $\vec{x}_1\overline{0}$ in $A_L^0$, we have that this is $h_1(v_1)$ where $h_1$ is the grading for the $1$-d Adinkra $A_L^0$.  On the other hand, in $A_L^0\times A_R^0$, this is $h_L(v_1,v_2)$.
