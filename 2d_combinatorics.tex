...
Finally, we put an admissable dashing on $A$. This is possible because $C$ is doubly even: one approach would be to use \cite{d2l:topology} to come up with a dashing on the $1$-d Adinkra $I^n/C$ and use $\Phi$ to pull this back to $A$.
\end{construction}
...
\com{I think we should remove this example. It is unclear what separate concept it illustrates, and the above example is already quite thorough. If we were to do another example, it may be good to do one where the grading isn't forced, for example.}
\begin{ex}
