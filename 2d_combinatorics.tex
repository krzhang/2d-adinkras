...
\begin{cor}
\label{cor:rectangle}
Let $A$ be a connected $2$-d Adinkra.  The support of $A$ is a rectangle.  That is, there exist integers $x$ and $y$ such that the support is
\[\{(x,y)\in\ZZ^2\,|\,0 \le x\le x\mbox{ and }0\le y\le y\}.\] All connected components of $A_L$ (and respectively $A_R$) are isomorphic as graded posets.
\end{cor}
\begin{proof}
By the definition of the product, the support of $A_L^0 \times A_R^0$, where $A_L^0$ has support $\{(0,i) | 0 \leq i < y\}$ and $A_R^0$ has support $\{(i,0) | 0 \leq i < x\}$, is exactly the given rectangle; taking quotients does not change the support, so this is the support of $A$ as well.

Let $A' = A_L^0 \times A_R^0$. All connected components of $A'_L$ (resp. $A'_R$) are isomorphic to $A_L^0$ (resp. $A_R^0$). Quotients preserve connected components and act the same way on each component, so all of the connected components of $A_L$ (resp. $A_R$) must be as well.
\end{proof}
