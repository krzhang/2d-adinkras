...
\Edge[color=blue](A)(E)
\Edge[color=blue](A')(E')
\Edge[color=red,style=dashed](B)(F)
\Edge[color=red,style=dashed](B')(F')
\Edge[color=green](B)(G)
\Edge[color=green](B')(G')
\Edge[color=blue,style=dashed](B)(H')
\Edge[color=blue,style=dashed](B')(H)
...
\Edge[color=blue](C)(G')
\Edge[color=blue](C')(G)
\Edge[color=black,style=dashed](D)(G)
\Edge[color=black,style=dashed](D')(G')
\Edge[color=red,style=dashed](D)(H)
\Edge[color=red,style=dashed](D')(H')
\Edge[color=blue](D)(F')
\Edge[color=blue](D')(F)
...
\end{tikzpicture}
\end{tabular}
\caption{The graphs $I^4$ and $G = I^4/\{0000,1111\}$. As the code $\{0000, 1111\}$ is doubly-even, there exists an Adinkra with $G$ as its underlying graph. \com{Kevin's version 2 has colors and dashings, and lines up the quotient as reflective symmetry} \label{fig:4cube folding}}
\end{center}
\end{figure}


%% same as above but without dashing
...
\end{tikzpicture}
\end{tabular}
\caption{The graphs $I^4$ and $G = I^4/\{0000,1111\}$. As the code $\{0000, 1111\}$ is doubly-even, there exists an Adinkra with $G$ as its underlying graph.\com{Kevin's version 3 with no dashing} \label{fig:4cube folding}}
\end{center}
\end{figure}


%% version 4: quotiented vertices near each other
...
\end{tikzpicture}
\end{tabular}
\caption{The graphs $I^4$ and $G = I^4/\{0000,1111\}$. As the code $\{0000, 1111\}$ is doubly-even, there exists an Adinkra with $G$ as its underlying graph. \com{Kevin's version 4 has identified vertices next to each other} \label{fig:4cube folding}}
\end{center}
\end{figure}

% version 5 is like version 4 but no dashed edges
...
\end{tikzpicture}
\end{tabular}
\caption{The graphs $I^4$ and $G = I^4/\{0000,1111\}$. As the code $\{0000, 1111\}$ is doubly-even, there exists an Adinkra with $G$ as its underlying graph. \com{Kevin's version 5 has identified vertices next to each other and no dashing} \label{fig:4cube folding}}
\end{center}
\end{figure}

% version 6 is more cube-like
...
\end{tikzpicture}
\end{tabular}
\caption{The graphs $I^4$ and $G = I^4/\{0000,1111\}$. As the code $\{0000, 1111\}$ is doubly-even, there exists an Adinkra with $G$ as its underlying graph. \com{Kevin's version 6 is more cube-like} \label{fig:4cube folding}}
\end{center}
\end{figure}
...
Next, we shall prove that for any tree $T$ that is a subgraph of $A$, there exists a vertex switch $F$ on $A$ so that $F(\mu)$ and $\bar{\mu}$ agree on $T$.  This can be proved by induction the number of vertices in $T$.  The base case of one vertex is trivial.  If $T$ has more than one vertex, then there is a leaf $v$ in $T$ connected to only one other vertex in $T$.  Let $e$ be the edge connecting $v$ to a vertex in $T$, and let $T_0$ be the tree with the vertex $v$ and edge $e$ omitted.  Then $T_0$ has one fewer vertex than $T$ so by the inductive hypothesis, there is a vertex switch $F_0$ on $A$ so that $F_0(\mu)$ and $\bar{\mu}$ agree on $T_0$.  If $F_0(\mu)(e)\not=\bar{\mu}(e)$, then let $F$ be the vertex switch $F_0$ followed by a vertex switch on $v$; otherwise let $F=F_0$.  Then $F(\mu)$ and $F_0(\mu)$ agree on $T_0$, so $F(\mu)$ agrees with $\bar{\mu}$ on all of $T$.
