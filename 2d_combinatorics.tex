...
Suppose $v_1$ is a vertex with bigrading $(x_1,y_1)$ and $v_2$ is a vertex with bigrading $(x_2,y_2)$.  Since $A$ is connected, there is an $\vec{x}\in\ZZ_2^n$ so that $\vec{x}v_1=v_2$.  Write $\vec{x}=\vec{x}_L+\vec{x}_R$, where $\vec{x}_L$ has only $1$s for left-moving colors and $\vec{x}_R$ has only $1$s for right-moving colors.  Then $\vec{x}_L(v_1)=\vec{x}_R(v_2)$ is a vertex that shares the $h_R$ of $v_1$ and $h_L$ of $v_2$, so that it has bigrading $(x_2,y_1)$; and $\vec{x}_L v_2=\vec{x}_R v_1$ likewise has bigrading $(x_1,y_2)$.
\end{proof}
...
Let $v$ be a vertex in the $2$-d Adinkra $A$, and let $g\in C(A)$.  Then $Z_L(\pi_L(g))v=Z_R(\pi_R(g))v$ and this has the same bigrading as $v$.
\end{prop}

\begin{proof}
If $g\in C(A)$, then $Z_R(\pi_R(g))(Z_L(\pi_L(g))v)=(Z_L(\pi_L(g))+Z_R(\pi_R(g)))=gv=v$, and by acting by $Z_R(\pi_R(g))$ we get $Z_L(\pi_L(g))v=Z_R(\pi_R(g))v$.  Since $Z_L(\pi_L(g))v$ has the same $h_R$ as $v$, and since $Z_R(\pi_R(g))v$ has the same $h_L$ as $v$, we have that $Z_L(\pi_L(g))v$ has the same bigrading as $v$.
\end{proof}
