...
\end{cor}
\begin{proof}
Let $A'=A_L^0\times A_R^0$ and $A'_L$ and $A'_R$ be the $1$-d Adinkras consisting of the left-moving (resp. right-moving) edges of $A'$.  From the definition of the product, and the fact that $A_L^0$ is connected, it is clear that the connected components of $A'_L$ are of the form $A_L^0\times\{w\}$ for some $w\in A_R^0$.  For such a $w$, let $\vec{x}\in\ZZ_2^q$ be so that $\vec{x}\overline{0}=w$ in $A_R^0$.  By Lemma~\ref{lem:phiformula}, $\Phi(v,w)=Z_R(\vec{x})v$, which is one-to-one as a function of $v$.  Therefore $\Phi$ restricted to $A_L^0\times\{w\}$ is an isomorphism onto its image.

Consider a connected component of $A_L$.  Since $\Phi$ is surjective, there is a connected component of $A'_L$ that maps to it under $\Phi$.  Both the connected component of $A_L$ and the image under $\Phi$ of the connected component of $A'_L$ are $1$-d Adinkras, and so they must be equal.
\end{proof}

\com{Note: this is harder than the following proof which doesn't use the main theorem at all:}
...
where $C_L=C\cap Z_L(\ZZ_2^p)$ and $C_R=C\cap Z_R(\ZZ_2^q)$.

Use the construction from \cite{d2l:omni,d2l:topology} to create the $1$-d Adinkras $A_1=I^p/\pi_L(C_L)$ and $A_2=I^q/\pi_R(C_R)$.  Use the ``hanging gardens'' construction in Ref.~\cite{d2l:graph-theoretic} on $A_1/\pi_L(K)$ and $A_2/\pi_R(K)$ to create a grading $h_1$ on $A_1$ that is invariant under $\pi_L(K)$ and a grading $h_2$ on $A_2$ that is invariant under $\pi_R(K)$.
