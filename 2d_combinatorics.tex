...
\end{itemize}
\end{definition}
Note that in [Reference], there is also a bipartition of the vertices.  This is rendered obsolete by the grading, since $h(v)$ is even if and only if $v$ is a boson.
...
If $(v,w)$ is an edge in the Adinkra, $(q_i(v),q_i(w))$ is an edge.  This follows from items 2 and 3 in the definition of an Adinkra above.  Therefore $q_i$ is a graph homomorphism.  From item 2 in the definition, we have that $q_i(q_i(v))=v$ for all $v\in V$, so $q_i$ is invertible and thus a graph isomorphism.  From item 3 in the definition we also have for all $i$ and $j$, $q_i(q_j(v))=q_j(q_i(v))$.  As a result, we can then define an action of $\ZZ_2^n$ on the graph $(V,E)$ underlying the Adinkra in the following way:
\[(x_1,\ldots,x_N)v=q_1^{x_1}\circ\cdots\circ q_n^{x_n}(v)\]


\subsection{The code for an Adinkra}
...
\end{prop}
\begin{proof}
Let $w\in V$.  By the previous proposition, there exists a $\gamma\in \ZZ_2^n$ so that $\gamma v=w$.
...
The following Adinkra was defined in [reference] and will be useful now.
\begin{definition}
...
\begin{thm}
There is a binary block code $K$ of length $n$ so that as colored graphs,
