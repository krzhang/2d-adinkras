...
The fact that $C$ acts on $I^n$ via graph homomorphisms results in a graph (see later section on Quotients).
...
\begin{prop}
If a group $C$ acts on a graph $\Gamma=(V,E)$ via graph isomorphisms, then $C$ acts on the set of edges in the following manner: if $c\in C$ and $(v,w)\in E$, then $c(v,w)=(cv,cw)$.
\end{prop}
\begin{proof}
The fact that $(cv,cw)$ is an edge follows directly from the definition of graph homomorphism.
