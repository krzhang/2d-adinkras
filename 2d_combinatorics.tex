...
Now let $v$ be any vertex of $A$.  There is a path from $\overline{0}$ to $v$.  Using Proposition~\ref{prop:reorderpath}, there exists a left-moving path $p_L$ from $\overline{0}$ to a vertex $x$ and a right-moving path $p_R$ from  $x$ to $v$.  Since $p_L$ is a left-moving path, $x\in A_L^0$.

Now since $p_R$ is right-moving and connects $i_3(x)$ with $i_3(v)$, we have $h_L(i_3(v))=h_L(i_3(x))=h_L(x,\overline{0})=h_L(x)$.  This is $h_L(v)$ because $p_R$ is right-moving and connects $x$ with $v$.  Thus, $h_L(i_3(v))=h_L(v)$.
